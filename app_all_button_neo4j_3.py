...
def submit():
    ...
    source_node_id = request.form.get("source_node_id")  # present when creating a relation
    relation_name = request.form.get("relation_name")    # property that defines relation
    ...
    for field in fields:
        if field.strip().lower() == "objekt-id":
            continue
    ...
    # 🔹 Call Java program for CREATE or RELATION
    try:
...
@app.route('/new_form')
def new_form():
    """Open a new blank form after saving data."""
    fields = get_field_list()
    data = {}
    colors = {}
    name_entities = {}
...
@app.route('/open_entity_form/<relation>/<source_node_id>/<entity_name>')
def open_entity_form(relation, source_node_id, entity_name):
    """Open a new form for a clicked Named Entity, passing relation info."""
    fields = get_field_list()
    data = {fields[0]: entity_name}
    colors = {}
    name_entities = {}
    return render_template(
        "objekt_form_buttons.html",
...
def success():
    return '''
    <h3>✅ Daten erfolgreich gespeichert, als CSV/Excel exportiert und in Neo4j importiert!</h3>
    <a href="/new_form">
        <button style="margin-top:20px; padding:10px 20px;">➕ Neues Objekt erfassen</button>
    </a>
    '''
