...
def add_data_to_neo4j_from_csv(csv_file):
    """Create one Neo4j node using Property->text pairs from CSV."""
    properties = {}
    ...
    with open(csv_file, mode="r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            val = row.get("text", "").strip() or "No data"
            # Skip only Objekt-ID row, include nodeType
            if prop and prop != "Objekt-ID":
    ...
    with driver.session() as session:
        ...
        print(f"✅ Created Neo4j node from CSV with ID: {node_id}")
    return node_id
...
def submit():
    ...
    for field in fields:
        if field.strip().lower() == "objekt-id":
            continue  # skip duplicate Objekt-ID row
